...
\bd
\item[``\href{./v6/file.h}{file.h}''] describes the structure
of the ``file'' array;

\item[``\href{./v6/filsvs.h}{filsvs.h}''] describes the structure
of the ``super block'' for ``mounted''
file systems;

\item[``\href{./v6/ino.h}{ino.h}''] describes the structure of
``inodes'' recorded on ``mounted''
devices;

\item[``\href{./v6/inode.h}{inode.h}''] describes the structure
of the ``inode'' array;
\ed

The next two files, ````\href{./v6/ken/{sys2.c}{sys2.c}'''' and
````\href{./v6/ken/{sys3.c}{sys3.c}'''' contain code for system calls.
(````\href{./v6/ken/{sys1.c}{sys1.c}'''' and ````\href{./v6/ken/{sys4.c}{sys4.c}'''' were presented
in Section Two).

Tne next five files, ````\href{./v6/ken/{rdwri.c}{rdwri.c}'''',
````\href{./v6/ken/{subr.c}{subr.c}'''', ````\href{./v6/ken/{fio.c}{fio.c}'''', ````\href{./v6/ken/{alloc.c}{alloc.c}'''' and
````\href{./v6/ken/{iget.c}{iget.c}'''', together present the principal routines for file management, and
provide a link between the i/o oriented
system calls and the basic i/o routines.

The file ````\href{./v6/ken/{nami.c}{nami.c}'''' is concerned with
searching directories to convert file
pathnames into ``inode'' references.

Finally, ````\href{./v6/ken/{pipe.c}{pipe.c}'''' is the ``device
driver'' for pipes.
